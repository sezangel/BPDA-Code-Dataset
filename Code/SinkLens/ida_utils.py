...
MIPS_SYSCALL_INSN = 'syscall'       
X86_SYSCALL_INSN = 'int     80h'    
ARM_SYSCALL_INSN = 'SVC     0'      
PPC_SYSCALL_INSN = 'sc'             
...
def x86_get_sys_int_value(ea):
    ...
    if mnem == 'mov' or mnem == 'movq':
        if get_operand_type(ea, 0) == o_reg and get_operand_value(ea, 0) == 0x0: 
...
def mips_get_sys_int_value(ea):
    ...
    sys_int_value = None
    if mnem == 'li':
        if get_operand_type(ea, 0) == o_reg and get_operand_value(ea, 0) == 0x2: 
            sys_int_value = get_operand_value(ea, 1)
...
def arm_get_sys_int_value(ea):
    ...
    sys_int_value = None
    if mnem.lower() == 'mov':
        if get_operand_type(ea, 0) == o_reg and get_operand_value(ea, 0) == 0x7: 
            sys_int_value = get_operand_value(ea, 1)
...
def has_loop(func_ea):
    ...
    try:
        ...
        res = list(nx.simple_cycles(func_cfg)) 
...
def filter_vex_str_dst(vex_str):
    ret_str = None
    if '(' in vex_str:
        pattern = r'\((.*?)\)'
...
def filter_vex_str_src(vex_str):
    ret_str = None
    if '(' in vex_str:
        pattern = r'\((.*?)\)'
        res = re.findall(pattern, vex_str)
        if len(res) != 1:
            ...
            if ',' in res:
                ret_str = res.split(',')[0].strip(' ')
                other_str=res.split(',')[1].strip(' ')
                pattern_1=r'^t\d+$'    
                match=re.match(pattern_1,other_str)
                if match:
                    ret_str=[ret_str,other_str]
            else:
                ret_str = res
            return ret_str
    else:
